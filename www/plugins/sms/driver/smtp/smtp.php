...
namespace plugins\sms\driver\smtp;
 ...
 * @example
 * $this->setServer("smtp@126.com", "XXXXX@126.com", "XXXXX"); //设置smtp服务器，普通连接方式
 * $this->setServer("smtp.gmail.com", "XXXXX@gmail.com", "XXXXX", 465, true); //设置smtp服务器，到服务器的SSL连接
 * $this->setFrom("XXXXX"); //设置发件人
 * $this->setReceiver("XXXXX"); //设置收件人，多个收件人，调用多次
 * $this->setCc("XXXX"); //设置抄送，多个抄送，调用多次
 * $this->setBcc("XXXXX"); //设置秘密抄送，多个秘密抄送，调用多次
 * $this->addAttachment("XXXX"); //添加附件，多个附件，调用多次
 * $this->setMail("test", "<b>test</b>"); //设置邮件主题、内容
 * $this->send(); //发送
 * 邮件发送类
 */
class smtp extends Factory {
/**
    ...
    private function socket() {
        ...
        if(!extension_loaded('sockets')){
            $this->_errorMessage = 'php_sockets not enabled';
            return false;
        }
     ...
     * @return void
     * @author Leo <QQ:13708867890>
     */
    public function sendSms($email, $templateData = [], $content = '',$nationCode = 86)
	{
        $subject = $content;
        $body = str_replace(array_keys($templateData),$templateData,$this->templateExample);
        $isSecurity = property_exists('smtp','isSecurity') == true ? $this->gatewayConfig['isSecurity'] : false ;
        $this->setServer($this->gatewayConfig['server'], $this->gatewayConfig['username'], $this->gatewayConfig['password'], $this->gatewayConfig['port'], $isSecurity); //设置smtp服务器，到服务器的SSL连接
        $this->setFrom($this->gatewayConfig['username']); //设置发件人
        $this->setFromName($this->gatewayConfig['fromusername']); //设置发件人姓名
        $this->debug(true);
        $this->setReceiver($email); //设置收件人，多个收件人，调用多次
        $this->setMail($content, $body); //设置邮件主题、内容
        $result = $this->send();
        return $result == true ? true : $this->_errorMessage;
    }
