...
// +----------------------------------------------------------------------
namespace app\pay\driver\alipay;
...
class alipay extends PayAbstract
{
    public function __construct($config = [])
    {
        ...
        $this->config['refund_method']  = 'alipay.trade.refund';

        if (request()->isMobile()) {
            $this->config['pay_method']     = 'alipay.trade.wap.pay';
            $this->config['product_code']   = 'QUICK_WAP_WAY';
        } else {
            $this->config['pay_method']     = 'alipay.trade.page.pay';
            $this->config['product_code']   = 'FAST_INSTANT_TRADE_PAY';
        }
    ...
    public function _submit($param)
    {
        ...
        $data['param']['product_code']      = $this->config['product_code'];
        ...
        // 公共请求参数
        $data['param'] = self::publicParam($data['param'], $this->config['pay_method']);
        $data['param']['sign'] = self::sign($data['param']);

        // 网关地址
        $data['url'] = $this->config['gateway_url'];
        // 提交方式
        $data['method'] = $this->config['gateway_method'];
    ...
    public function _refundSubmit($param = [])
    {
        ...
        // 公共请求参数
        $data['param'] = self::publicParam($data['param'], $this->config['refund_method']);
        ...
        // 发送请求并返回结果
        $result = Http::postRequest($this->config['gateway_url'], $data['param']);
     ...
     * @return bool|string
     */
    private function publicParam($param, $method = 'alipay.trade.page.pay')
    {
        $data = [];
