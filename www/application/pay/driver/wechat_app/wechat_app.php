...
// +----------------------------------------------------------------------
namespace app\pay\driver\wechat_app;

use app\pay\driver\Wechat;
use app\pay\model\PayLog as PayLogModel;
use think\view;

class wechat_app extends Wechat
{
    ...
    public function _submit($param = [])
    {
        ...
        $tplData['partnerid']   = $this->config['mch_id'];
        $tplData['prepayid']    = $result['prepay_id'];
        $tplData['package']     = 'Sign=WXPay';
        $tplData['noncestr']    = random(16, 0);
        $tplData['timestamp']   = $_SERVER['REQUEST_TIME'];
        $tplData['sign']        = $this->wechatSign($tplData, $this->config['key']);
